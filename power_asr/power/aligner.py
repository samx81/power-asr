...
class PowerAligner:
    ...
    def align(self):
        ...
        assert self.hypwords == self.power_alignment.s2_string(), "hyp mismatch:\n{0}\n{1}".format(
            self.hypwords, self.power_alignment.s2_string())
        assert self.refwords == self.power_alignment.s1_string(), "ref mismatch:\n{0}\n{1}".format(
            self.refwords, self.power_alignment.s1_string())
    ...
    def phoneAlignToWordAlign(cls, ref_words, hyp_words, ref_phones, hyp_phones, break_on_syllables=True):
        ...
        while worklist:
            ...
            ref_word_iter = enumerate(ref_words[ref_word_span[0]:ref_word_span[1]]
            ...
            # Indicates whether a word is already accounted for in the alignment when a phoneme is reached.
